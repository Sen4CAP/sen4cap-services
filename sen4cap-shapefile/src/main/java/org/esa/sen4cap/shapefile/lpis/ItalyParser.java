 ...
 * @author Cosmin Cara
 */
@LPIS(countryCode = "ITA")
public class ItalyParser extends Parser<Parcel> {
    ...
    protected Parcel parseFeature(SimpleFeature feature, MathTransform reprojection) throws Exception {
        ...
        try {
            ...
            parcel.setIdentifier(feature.getAttribute("Province").toString() + "-" +
                                 feature.getAttribute("Id_island").toString());
            parcel.setArea((Double) feature.getAttribute("Area"));
        } catch (Exception ex) {
